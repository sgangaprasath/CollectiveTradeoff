...
trails over which the collective travels.
...
In this article we are interested in understanding how and when an
agent can innovate from its innate behavior to follow pheromone trails to explore the environment and find novel solutions
to the navigation problem. In order to understand the dynamics of innovation, we assume that the agent has already
traversed the trajectory $\x^*(s)$ before.
...
an Ornstein-Uhlenbeck (OU) process. This explorative policy $\pi_{\OU}(\r)$ can be written through $\P(\theta, t)$
the probability that the agent will take a particular angle $\theta$ at time $t$ or distance $s$. However since the orientation of the
agent is dependent on $\r(s_c)$, the angle between agent location $\r(s_c)$ to target A can be denoted
as $\phi$ and $\theta=\phi+\varphi$ where $\varphi$ is the fluctuation around this orientation that follows
an OU-process. The probability $\P(\varphi, s)$ can then be written as
\[
...
is farther from the target A. We assume that the agent reaching a distance $\sigma$ from the end-point A can reach the
target and thus the reward is a function of the propagation of variance in agent's position towards the
...
\begin{align}
    ...
    & \ + 4 \e^{-\nu (s-s_c)/v_o} - 3 \bigg].
\end{align}
Assuming that the agent reaching a distance $\sigma$ along $\ebh_\perp$ from the end-point A can reach the
target (this is associated with the agent's capacity to sense the food source/target),
the probability that an agent will reach the target is given by $\P(||r_\perp^{s^*} - r^*|| \leq \sigma)
= \sigma/\psi(s^*)$. We can now calculate the value function, $v_{\rm OU}(\r, \theta)$ using the formula
\[
    v_{\rm OU}(\r) = \int_{s_c}^{s^*} \int \D[\theta(s)] \P_{\rm OU}[\theta(s)] R(s) \ \d s.
\]
Let us assume that the reward function for the OU process is $R(s) = \gamma[1 - \Theta(||r_\perp(s^*) - r^*|| - \sigma)]$.
From this we see that the value of $v_{\rm OU}(\r^c) = \sigma \gamma/\psi(s^*)$.
