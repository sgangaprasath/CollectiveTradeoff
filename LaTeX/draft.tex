...
\]
where the bending energy $E_b[\theta(s)] = B/2 \int_0^{s_c} (\theta'(s)-\kappa_o)^2 \ \d s$, fugacity
...
We are interested in evaluating the value function of this policy $\pi_{\rm int}(\r)$
denoted by $v_{\rm int}(\r^t)$ whose expression is
\[
