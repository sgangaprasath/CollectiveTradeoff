...
\begin{align}
    ...
    & \ + 4 \e^{-\nu (s-s_c)/v_o} - 3 \bigg].
\end{align}
As already mentioned, the agent reaching a distance $\sigma$ along $\ebh_\perp$ from the end-point A can reach the
target, the probability that an agent will reach the target is given by $\P(||r_\perp^{s^*} - r^*|| \leq \sigma)
...
which simply means that the agent reaching with a distance $\sigma$ gets a reward $\mu$.
From this we see that the value of $v_{\rm OU}(\r^c) = \sigma \mu/\psi(s^*)$.
...
\[
    v(\r; s_c, D) = \frac{\sigma \mu}{\psi(s^*; s_c, D)} + \frac{\phi^*}{\kappa_o} [ \e^{-\kappa_os/\phi^*} - \e^{-\kappa_o s_c/\phi^*}],
\]
