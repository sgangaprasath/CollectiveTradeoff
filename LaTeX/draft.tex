...
for further non-dimensionalization such that $s \rightarrow l_\nu \hat{s}$. We drop the hat in what follows and we get
the non-dimensional variance, $\psi(s)$ (after setting $\psi(s_c) =0$) as a function of non-dimensional distance, $s$ as
\begin{align}
...
can reach the target, the probability that an agent will reach the target is given by $\P(||r_\perp^{s^*} - r^*|| \leq \sigma)
